...
export const authJsonFetch = async (
  url,
...
export const authFormFetch = async (
  url,
...
export const adminApi = {
  skills: {
    ...
    update: (token, id, formData) =>
      authFormFetch(`${SKILLS_URL}/${id}`, { method: "PUT", token, formData }),
    remove: (token, id) =>
      authJsonFetch(`${SKILLS_URL}/${id}`, { method: "DELETE", token }),
  },
  portfolios: {
    list: (token) => authJsonFetch(PORTFOLIOS_URL, { token }),
    create: (token, formData) =>
      authFormFetch(PORTFOLIOS_URL, { token, formData }),
    update: (token, id, formData) =>
      authFormFetch(`${PORTFOLIOS_URL}/${id}`, {
        method: "PUT",
        token,
        formData,
      }),
    remove: (token, id) =>
      authJsonFetch(`${PORTFOLIOS_URL}/${id}`, { method: "DELETE", token }),
  },
  messages: {
    list: (token) => authJsonFetch(MESSAGES_URL, { token }),
    remove: (token, id) =>
      authJsonFetch(`${MESSAGES_URL}/${id}`, { method: "DELETE", token }),
  },
  localizations: {
    listFlat: (token) => authJsonFetch(`${LOCALIZATIONS_URL}/flat`, { token }),
    upsert: (token, body) =>
      authJsonFetch(`${LOCALIZATIONS_URL}/upsert`, {
        method: "POST",
        token,
        body,
      }),
  ...
  employees: {
    ...
    regenerate: (token, body) =>
      authJsonFetch(`${EMPLOYEES_URL}/regenerate`, {
        method: "POST",
        token,
        body,
      }),
  ...
  blogs: {
    ...
    create: (token, body) =>
      authJsonFetch(BLOGS_URL, { method: "POST", token, body }),
    update: (token, id, body) =>
      authJsonFetch(`${BLOGS_URL}/${id}`, { method: "PUT", token, body }),
    remove: (token, id) =>
      authJsonFetch(`${BLOGS_URL}/${id}`, { method: "DELETE", token }),
  },
  currentFocus: {
    ...
    update: (token, id, body) =>
      authJsonFetch(`${CURRENT_FOCUS_URL}/${id}`, {
        method: "PUT",
        token,
        body,
      }),
    remove: (token, id) =>
      authJsonFetch(`${CURRENT_FOCUS_URL}/${id}`, { method: "DELETE", token }),
  },
};
