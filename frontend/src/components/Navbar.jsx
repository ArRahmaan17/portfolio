...
const HOME_SECTION_IDS = ["stack", "portfolio", "contact"];

export default function Navbar({
  classNavbar = "bg-transparent",
  changeTheme,
  theme,
  changeLanguage,
  lang = "en",
  ...
  useEffect(() => {
    ...
    const updateActiveSection = () => {
      ...
      animationFrame = window.requestAnimationFrame(() => {
        ...
        let nextSection = isAtPageEnd ? "contact" : "";
        if (!isAtPageEnd && window.scrollY > 100) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50" role="banner">
      <div
        className={`mx-auto px-3 transition-all duration-300 sm:px-5 ${compact ? "max-w-6xl pt-3" : "max-w-7xl pt-4"}`}
      >
        <PillNav className="w-full justify-between !px-2.5 !py-2 sm:!px-3">
          <button
            onClick={goHome}
            className="group flex items-center gap-2 rounded-full pr-2"
            aria-label="Ardhi Rahmaan, go to home"
          >
            <img
              className="h-9 w-9 rounded-full object-contain transition-transform group-hover:rotate-[-6deg]"
              src={logo}
              alt=""
            />
            <span className="hidden font-display text-xs font-semibold tracking-tight text-slate-900 dark:text-cloud sm:block">
              Ardhi Rahmaan
            </span>
          </button>
          ...
          >
            <button onClick={goHome} className={navClass}>
              {t("home")}
            </button>
            {isHome
              ? homeLinks.map((item) => (
                  <Link
                    ...
                    smooth
                    offset={-72}
                    className={`${navClass} ${activeSection === item.to ? "nav-link-active" : ""}`}
                  >
                    {item.label}
                  </Link>
                ))
            ...
            <a
              href="/blog"
              className={`${navClass} ${window.location.pathname.startsWith("/blog") ? "nav-link-active" : ""}`}
            >
              Blog
            </a>
          </nav>
          ...
          <div className="flex items-center gap-1.5">
            <div ref={controlsRef} className="relative hidden lg:block">
              <button
                ...
                className="flex h-9 items-center gap-2 rounded-full bg-slate-100/80 px-3 font-mono text-[0.65rem] font-medium uppercase tracking-wider text-slate-700 transition hover:bg-slate-200 dark:bg-white/10 dark:text-slate-200 dark:hover:bg-white/15"
              ...
              {controlsOpen ? (
                <div className="absolute right-0 mt-2 w-52 overflow-hidden rounded-2xl border border-slate-200 bg-white/95 p-2 shadow-2xl backdrop-blur-2xl dark:border-white/10 dark:bg-panel/95">
                  <p className="px-3 py-2 font-mono text-[0.62rem] uppercase tracking-[0.16em] text-slate-400">
                    Language
                  </p>
            ...
            <button
              ...
              className="icon-action !h-9 !w-9 lg:hidden"
      ...
      {mobileOpen ? (
        ...
        >
          <button
            className="absolute inset-0 bg-void/45 backdrop-blur-sm"
          ...
          <div className="absolute inset-y-0 right-0 flex w-[min(88vw,24rem)] flex-col bg-white p-6 shadow-2xl dark:bg-void">
            <div className="flex items-center justify-between">
              <p className="font-display text-sm font-semibold text-slate-900 dark:text-cloud">
                Navigate
              </p>
              <button
                onClick={() => setMobileOpen(false)}
            ...
            <nav className="mt-12 flex flex-col" aria-label="Mobile navigation">
              <button onClick={goHome} className="mobile-nav-link">
                {t("home")}
              </button>
              {isHome
                ? homeLinks.map((item) => (
              ...
              <div className="flex gap-2">
                {LANGUAGES.map(({ code }) => (
                  <button
                    key={code}
                    onClick={() => selectLanguage(code)}
                    className={`rounded-full px-4 py-2 font-mono text-xs font-medium uppercase ${lang.startsWith(code) ? "bg-primary text-primary-contrast" : "bg-slate-100 text-slate-600 dark:bg-white/10 dark:text-slate-300"}`}
                  >
                    {code}
                  </button>
                ))}
